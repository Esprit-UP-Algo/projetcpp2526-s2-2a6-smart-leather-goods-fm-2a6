    ...
    , m_seuilTemperatureMax(35.0)
{
    ...
    // Ajouter les colonnes Arduino à MATIERES_PREMIERES si nécessaire
    ajouterColonnesArduino();
...
void ArduinoWidget::ajouterColonnesArduino()
{
    Connexion *cnx = Connexion::getInstance();
    if (!cnx->estConnecte()) {
        qDebug() << "⚠️ Pas de connexion BDD - colonnes Arduino non ajoutées";
        return;
    }
    
    qDebug() << "🔧 Vérification des colonnes Arduino dans MATIERES_PREMIERES...";
    
    // Liste des colonnes à ajouter
    QStringList colonnes = {
        "TEMPERATURE_ACTUELLE NUMBER(5,2) DEFAULT NULL",
        "HUMIDITE_ACTUELLE NUMBER(5,2) DEFAULT NULL",
        "DATE_DERNIERE_MESURE TIMESTAMP DEFAULT NULL",
        "STATUT_CONDITIONS VARCHAR2(20) DEFAULT 'NON_SURVEILLE'",
        "ALERTE_ACTIVE NUMBER(1) DEFAULT 0",
        "MESSAGE_ALERTE VARCHAR2(200) DEFAULT NULL",
        "TEMP_MIN_RECOMMANDEE NUMBER(5,2) DEFAULT 15.0",
        "TEMP_MAX_RECOMMANDEE NUMBER(5,2) DEFAULT 25.0",
        "HUMIDITE_MIN_RECOMMANDEE NUMBER(5,2) DEFAULT 40.0",
        "HUMIDITE_MAX_RECOMMANDEE NUMBER(5,2) DEFAULT 60.0",
        "DUREE_ALERTE_HEURES NUMBER(10,2) DEFAULT 0",
        "NOMBRE_ALERTES_TOTAL NUMBER(10) DEFAULT 0"
    };
    ...
    QStringList nomsColonnes = {
        ...
        "ALERTE_ACTIVE",
        "MESSAGE_ALERTE",
        "TEMP_MIN_RECOMMANDEE",
        "TEMP_MAX_RECOMMANDEE",
        "HUMIDITE_MIN_RECOMMANDEE",
        "HUMIDITE_MAX_RECOMMANDEE",
        "DUREE_ALERTE_HEURES",
        "NOMBRE_ALERTES_TOTAL"
    };
    
    int colonnesAjoutees = 0;
    int colonnesExistantes = 0;
    ...
    for (int i = 0; i < nomsColonnes.size(); ++i) {
        ...
        checkQuery.bindValue(":colonne", nomsColonnes[i]);
        ...
        if (checkQuery.exec() && checkQuery.next()) {
            ...
            if (count > 0) {
                ...
                colonnesExistantes++;
                qDebug() << "  ✓" << nomsColonnes[i] << "existe déjà";
            } else {
                // Ajouter la colonne
                QSqlQuery alterQuery(cnx->getDatabase());
                QString sql = QString("ALTER TABLE MATIERES_PREMIERES ADD %1").arg(colonnes[i]);
                
                if (alterQuery.exec(sql)) {
                    colonnesAjoutees++;
                    qDebug() << "  ✅ Colonne ajoutée:" << nomsColonnes[i];
                } else {
                    qDebug() << "  ❌ Erreur ajout" << nomsColonnes[i] << ":" << alterQuery.lastError().text();
                }
            }
        } else {
            qDebug() << "  ⚠️ Erreur vérification" << nomsColonnes[i] << ":" << checkQuery.lastError().text();
        }
    }
    
    // Ajouter les contraintes CHECK si nécessaire
    if (colonnesAjoutees > 0) {
        QSqlQuery constraintQuery(cnx->getDatabase());
        
        // Contrainte pour STATUT_CONDITIONS
        QString addConstraint1 = R"(
            ALTER TABLE MATIERES_PREMIERES 
            ADD CONSTRAINT chk_statut_conditions 
            CHECK (STATUT_CONDITIONS IN ('OPTIMAL', 'ALERTE_HUMIDITE', 'ALERTE_TEMPERATURE', 'CRITIQUE', 'NON_SURVEILLE'))
        )";
        
        if (!constraintQuery.exec(addConstraint1)) {
            // Ignorer l'erreur si la contrainte existe déjà
            if (!constraintQuery.lastError().text().contains("already exists")) {
                qDebug() << "  ⚠️ Contrainte STATUT_CONDITIONS:" << constraintQuery.lastError().text();
            }
        }
        
        // Contrainte pour ALERTE_ACTIVE
        QString addConstraint2 = R"(
            ALTER TABLE MATIERES_PREMIERES 
            ADD CONSTRAINT chk_alerte_active 
            CHECK (ALERTE_ACTIVE IN (0, 1))
        )";
        
        if (!constraintQuery.exec(addConstraint2)) {
            // Ignorer l'erreur si la contrainte existe déjà
            if (!constraintQuery.lastError().text().contains("already exists")) {
                qDebug() << "  ⚠️ Contrainte ALERTE_ACTIVE:" << constraintQuery.lastError().text();
            }
        }
    }
    ...
    // Résumé
    qDebug() << "📊 Résumé:";
    qDebug() << "  - Colonnes existantes:" << colonnesExistantes;
    qDebug() << "  - Colonnes ajoutées:" << colonnesAjoutees;
    
    if (colonnesAjoutees > 0) {
        qDebug() << "✅ Colonnes Arduino ajoutées avec succès à MATIERES_PREMIERES";
    } else if (colonnesExistantes == nomsColonnes.size()) {
        qDebug() << "✅ Toutes les colonnes Arduino existent déjà";
    }
}
